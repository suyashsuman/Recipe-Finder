...
const App = () => {
  const APP_ID = '61eda2ed';
  const APP_KEY = '121b63ae953f9ab4c1029bc1c3a7daed';
  ...
  const getRecipesFunction = async () => {
    try {
      ...
      setFoodRecipes(data.hits || []); // Ensure hits is always an array
    } catch (error) {
      console.error('Error fetching recipes:', error);
      setFoodRecipes([]); // Fallback to an empty array on error
    }
  };
